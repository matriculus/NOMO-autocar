...
out_size = (320*2, 180*2)

fourcc = cv2.VideoWriter_fourcc(*'mp4v') # Be sure to use lower case
output = "Video.mp4"
out = cv2.VideoWriter(output, fourcc, 20.0, out_size)
...
with detection_graph.as_default():
	with tf.Session(graph=detection_graph) as sess:
		...
		while True:
			...
			# print(n)
			image_np = cv2.resize(np.array(ImageGrab.grab(bbox=(0,45, 1280,720+45)).convert('RGB')), out_size)
			image_t = image_np
			...
			# 	else:
			# 		pass
			# 		# cv2.putText(image_t, "Tracking failure detected", (100, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
			# fps = cv2.getTickFrequency() / (cv2.getTickCount() - timer)
			# cv2.putText(image_t, "FPS: {}".format(fps), (100, 50), cv2.FONT_HERSHEY_SIMPLEX,0.5, (50, 170, 50), 2)
